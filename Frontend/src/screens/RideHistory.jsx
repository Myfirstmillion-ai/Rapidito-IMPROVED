...
import GestureHandler from "../components/common/GestureHandler";
import LoadingIndicator from "../components/common/LoadingIndicator";
import { springs, variants, gestures, parallaxBackground } from "../utils/animationUtils";
...
function RideHistory() {
  ...
  // Estado para gestos y animaciones
  const [refreshing, setRefreshing] = useState(false);
  const [activeRide, setActiveRide] = useState(null); // Para efectos visuales al tocar
  const { scrollY } = useScroll();
  
  // Efectos de parallax y animaciones basadas en scroll
  const headerOpacity = useTransform(scrollY, [0, 100], [1, 0.98]);
  const headerScale = useTransform(scrollY, [0, 100], [1, 0.98]);
  const statsTranslateY = useTransform(scrollY, [0, 100], [0, -10]);
  ...
  // Simular una actualización (pull-to-refresh)
  const handlePullRefresh = () => {
    setRefreshing(true);
    setTimeout(() => {
      setRefreshing(false);
      // Aquí iría la lógica de recarga de datos real
    }, 1500);
  };
    ...
    <div className="min-h-screen bg-gradient-to-br from-[#0A0A0A] via-[#101010] to-[#080808] overflow-y-auto">
      ...
      {refreshing && (
        <div className="absolute top-20 left-0 right-0 z-30 flex justify-center">
          <LoadingIndicator type="spinner" size="small" text="Actualizando..." />
        </div>
      )}
      
      {/* Header - iOS Deluxe con efectos de scroll */}
      <motion.div
        style={{ 
      ...
      >
        <div className="px-6 py-4">
          ...
          {/* Stats Cards - con animación parallax */}
          <motion.div 
            style={{ y: statsTranslateY }}
      ...
      {/* Contenedor principal con pull-to-refresh */}
      <GestureHandler
        direction="vertical"
      ...
      >
        {/* Contenido principal - con estados de carga */}
        {loading ? (
          <div className="space-y-6">
            <SkeletonLoader type="card" showHeader count={1} />
            <SkeletonLoader type="ride" count={3} />
          </div>
        ) : (
          <motion.div 
          ...
          >
            {rides.map((ride) => (
              <RideCard
                ...
                isActive={activeRide === ride._id}
                setActive={setActiveRide}
                onToggle={() =>
...
export const RideCard = ({ 
  ...
  return (
    <GestureHandler
      ...
      onSwipeLeft={() => console.log('Swipe izquierda - Archivar')}
      onSwipeRight={() => console.log('Swipe derecha - Detalle')}
      dragElastic={0.5}
    ...
    >
      <Card
        variant="glass"
        className={`overflow-hidden backdrop-blur-md border border-white/20 ${isActive ? 'ring-2 ring-[#10B981] ring-offset-2 ring-offset-black' : ''}`}
      >
        <motion.div
        ...
        {/* Indicador de swipe */}
        <div className="px-4 py-1 flex justify-between text-[10px] text-white/30">
          <span className="flex items-center">
            <ArrowLeft size={10} className="mr-1" /> Archivar
          </span>
          <span className="flex items-center">
            Ver detalle <ArrowLeft size={10} className="ml-1 transform rotate-180" />
          </span>
        </div>
      </Card>
    </GestureHandler>
